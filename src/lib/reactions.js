...
// photoId null = the check-in itself.
export function summarizeReactions(rows, userId, photoId = null) {
  const forTarget = rows.filter((r) =>
    photoId === null ? r.photo_id === null : r.photo_id === photoId
  );
  const counts = {};
  let mine = null;
...
export async function toggleReaction({ activityId, photoId = null, userId, emoji }) {
  let q = supabase
    .from("activity_reactions")
    .select("id, emoji")
    .eq("activity_id", activityId)
